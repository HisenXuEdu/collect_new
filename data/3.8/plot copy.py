...
# 读取pkl
data = pd.read_pickle('./data/3.8/pose2/gao.pkl')
force_list = data[0]
pose_list = data[1]
emg_list = data[2]
...
def calc_impedance_without_d(x, fx, y, fy, z, fz):
    """
    计算阻抗参数
    :param x: 位置、速度、加速度数据
    :param y: 力数据
...
# 读取merge.pkl
data1 = pd.read_pickle('./data/3.8/merge_new.pkl')
...
for i in range(1, 4):
    emg_list = data[2]
    emg_list = np.array(emg_list[i])
    merge.append(emg_list)
    k.append([100, 100, 100])
...
k.append([kx, ky, kz])
# 将merge和k保存为pkl
pd.to_pickle([merge, k], './data/3.8/merge_new1.pkl')
print("数据保存成功！")